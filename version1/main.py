...
from special_moves import Chadwick, Serenity, Zohphia, Kana, Shizuo, Rabadon
...
class Battle:
  ...
  def __init__(self, player_party: list[GachaCharacter],
               enemy_party: list[Enemy]) -> None:
          ...
          if player_choice == "1":
            ...
            for enemy in self.enemy_party:
              if not enemy.is_alive:
                ...
                enemies += enemy.name + ", "
            print(
                "Which enemy would you like to attack? Dead enemies are marked with an {X}. "
            )
            print(enemies)
            pick = input(
                "Enter the index of the enemy you wish to attack (First enemy is 0): "
            )
            ...
            for move in chrt.moveset:
              if chrt.moveset[0] == move:
                ...
                i += 1
            p_choice = input(
                "Which move would you like to use? Enter the number of the move you wish to use: "
            )
            while p_choice not in [
            ...
            if p_choice == "1":
              if chrt.moveset[1].cooldown == 0 and chrt.moveset[1].is_aoe:
                chrt.attack_aoe(list[Character](self.enemy_party),
                                chrt.moveset[1])
              else:
                enemies = ""
                for enemy in self.enemy_party:
                  if not enemy.is_alive:
                    ...
                    enemies += enemy.name + ", "
                pick = input(
                    "Which enemy would you like to attack? Dead enemies are marked with an {X}. Enter the index of the enemy you wish to attack (First enemy is 0): "
                )
                print(enemies)
                while pick not in [
                    str(i) for i in range(len(self.enemy_party))
                ] or not self.enemy_party[int(pick)].is_alive:
                  pick = input("That's not a valid choice! Choose again: ")
                chrt.attack_single(self.enemy_party[int(pick)],
                                   chrt.moveset[1])
            else:
...
def main():
  ...
  # attack = Move("Attack", "Attacks a single enemy for 100% of character attack.", 200, 0, True)
  # single = Move("IKUUUUUUUUUU", "The dragon's rage", 50, 0 , True)

  # aoe_skill = Move("AoE Attack", "Attacks all enemies for 100% of character attack", 90, 0, True)

  # char = GachaCharacter("Player 1", 10000, 50, 50, 50, 50, 50, True, [attack, single, aoe_skill], .5)

  # char2 = GachaCharacter("Player 2", 10000, 50, 50, 50, 50, 50, True, [attack, single, aoe_skill], .5)

  # char3 = GachaCharacter("Player 3", 10000, 50, 50, 50, 50, 50, True, [attack, single, aoe_skill], .5)

  # char4 = GachaCharacter("Player 4", 10000, 50, 50, 50, 50, 50, True, [attack, single, aoe_skill], .5)

  # bandit1 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True, [Move("Single Target", "attack character", 100, 0, False)])

  # bandit2 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True, [Move("Single Target", "attack character", 100, 0, False)])

  # bandit3 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True,
  #   [Move("Single Target", "attack character", 100, 0, False)])

  # bandit4 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True,
  #   [Move("Single Target", "attack character", 100, 0, False)])

  # bandit5 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True,
  #   [Move("Single Target", "attack character", 100, 0, False)])

  # bandit6 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True,
  #   [Move("Single Target", "attack character", 100, 0, False)])

  # bandit7 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True,
  #   [Move("Single Target", "attack character", 100, 0, False)])

  # bandit8 = Enemy("Bandit", 100, 20, 20, 20, 20, 2, True,
  #   [Move("Single Target", "attack character", 100, 0, False)])

  bandit1 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
    [Move("Single Target", "attack character", 1, 0, False)])

  bandit2 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
      [Move("Single Target", "attack character", 1, 0, False)])

  bandit3 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
                  [Move("Single Target", "attack character", 1, 0, False)])

  bandit4 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
                  [Move("Single Target", "attack character", 1, 0, False)])

  bandit5 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
                  [Move("Single Target", "attack character", 1, 0, False)])

  bandit6 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
                  [Move("Single Target", "attack character", 1, 0, False)])

  bandit7 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
                  [Move("Single Target", "attack character", 1, 0, False)])

  bandit8 = Enemy("Bandit", 1000, 2, 2, 20, .20, 2, True,
                  [Move("Single Target", "attack character", 1, 0, False)])

  a_player_party = []
  for x in range(4):
    select_char = input(
        "Choose a character: Chadwick, Serenity, Zohphia, Kana, Shizuo, Rabadon: "
    )
